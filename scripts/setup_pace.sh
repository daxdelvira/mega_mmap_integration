...
if [ ! -f "$INSTALL_ROOT/lib/libhermes.so" ]; then
    ...
    # v0.0.0-alpha hermes_shm does not install HermesShmCommonConfig.cmake even
    # though HermesShmConfig.cmake include()s it.  Create a stub so cmake doesn't
    # error out before it even starts configuring Hermes itself.
    _HERMES_SHM_COMMON_CFG="$INSTALL_ROOT/cmake/HermesShmCommonConfig.cmake"
    if [ ! -f "$_HERMES_SHM_COMMON_CFG" ]; then
        mkdir -p "$(dirname "$_HERMES_SHM_COMMON_CFG")"
        cat > "$_HERMES_SHM_COMMON_CFG" << 'HSMCFGEOF'
# Stub generated by setup_pace.sh.
# HermesShmCommonConfig.cmake was not installed by hermes_shm v0.0.0-alpha.
# thallium (Argobots/Mercury/Margo) is unavailable on PACE — treat as optional.
find_package(thallium CONFIG QUIET)
HSMCFGEOF
        echo "    Created HermesShmCommonConfig.cmake stub"
    fi
