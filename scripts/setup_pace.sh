...
# data_structure.h is a pure umbrella header added in a later commit;
# v0.0.0-alpha doesn't include it but grc-iit/hermes@master needs it.
_DATA_STRUCT_H="$INSTALL_ROOT/include/hermes_shm/data_structures/data_structure.h"
if [ ! -f "$_DATA_STRUCT_H" ]; then
    echo "==> Injecting missing data_structure.h umbrella header..."
    cat > "$_DATA_STRUCT_H" <<'HSHM_STUB'
/* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
