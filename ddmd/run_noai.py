"""
run_noai.py — DeepDriveMD (non-agentic / openmm_noai) with vanilla MegaMmap.

Same structure as run_agentic.py but drives the DeepDriveMD_OpenMM_NoAI
workflow (simulations only, no CVAE steering).  MegaMmap patch is still
installed so that any incidental contact-map loads also benefit.

Usage
-----
    MEGA_SHIM=/path/to/ddmd_contact_map_loader \
    MEGA_WINDOW=4g \
    python run_noai.py -c /path/to/experiment.yaml
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="DeepDriveMD non-agentic + vanilla MegaMmap")
    ...
    from deepdrivemd.workflows.openmm_noai import (
        ExperimentSettings,
        DeepDriveMD_OpenMM_NoAI,
        run_simulation,
        run_train,
        run_inference,
    )
    ...
    thinker = DeepDriveMD_OpenMM_NoAI(
        queue=queues,
